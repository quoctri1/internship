...
{
    int sd;
    unsigned char buffer_send[BUF_SIZE];
    unsigned char buffer_rec[BUF_SIZE];
    unsigned char source_ip[4] = {10,230,2,87};
    unsigned char target_ip[4] = {192,168,81,31};
    struct ifreq ifr;
    struct ethhdr *send_req = (struct ethhdr *)buffer_send;
    struct ethhdr *rcv_resp= (struct ethhdr *)buffer_rec;

    struct arp_header *arp_req = (struct arp_header *)(buffer_send+ETH2_HEADER_LEN);
    struct arp_header *arp_resp = (struct arp_header *)(buffer_rec+ETH2_HEADER_LEN);
    struct sockaddr_ll socket_address;
    int index,ret,length=0,ifindex;

    memset(buffer_send,0x00,60);
    ...
    snprintf(ifr.ifr_name, 10, "%s", "enp3s0");
    ...
    printf("Successfully got enp3s0 MAC address: %02x:%02x:%02x:%02x:%02x:%02x\n",
    ...
    buffer_send[32]=0x00;
    ret = sendto(sd, buffer_send, 42, 0,
    ...
    while (1) {
        memset(buffer_rec,0x00,60);

        length = recvfrom(sd, buffer_rec, BUF_SIZE, 0, NULL, NULL);
        if (length < 0) {
            perror("recvfrom()");
            exit(1);
        }
